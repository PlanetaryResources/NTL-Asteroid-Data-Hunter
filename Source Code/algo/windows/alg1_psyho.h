...
struct RandomForestConfig {
	...
	VI randomFeatures = vector<int>(tmp1, tmp1 + 1);
	VI randomPositions = vector<int>(tmp2, tmp2 + 1);
...
BOOST_CLASS_TRACKING(RandomForest, boost::serialization::track_never);
VVF trainFeatures;
VVF testFeatures;
VF trainResults;
	...
	vector<string> getAnswer() {
		...
		REP(i, vp.SZ) {
			...
			FOR(j, 3, 11) {
				...
				sprintf_s(str, sizeof(str), " %.10f", testFeatures[sampleID][j]);
				s += str;
			}
			s += " 0";
			sprintf_s(str, sizeof(str), " %.10f", score);
			s += str;
			sprintf_s(str, sizeof(str), " %.10f", (testFeatures[sampleID][21] + testFeatures[sampleID][22] + testFeatures[sampleID][23] + testFeatures[sampleID][24]) / 4);
			s += str;
			rv.PB(s);
		}
